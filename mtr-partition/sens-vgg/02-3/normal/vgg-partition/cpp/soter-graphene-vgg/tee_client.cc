...
struct vgg19 : public torch::nn::Module
{
    ...
    torch::Tensor forward2_new(torch::Tensor x) {
        ...
        x = F::relu(x);
        torch::Tensor x1 = F::relu(x);
        torch::Tensor x2 = F::relu(x);
        return x;
    }
    torch::Tensor forward3_new(torch::Tensor x) {
        // std::cout<<"forward2_new"<<std::endl;
        x = torch::max_pool2d(x, 2, 2, 0);
        torch::Tensor x1 = torch::max_pool2d(x, 2, 2, 0);
...
int main(int argc, char** argv) {
    ...
    torch::Tensor input = torch::rand({3, 3, 224, 224},torch::dtype(torch::kFloat32));
    torch::Tensor output;
